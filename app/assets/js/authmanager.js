 ...
 * This uses the same algorithm as Minecraft for offline mode UUIDs.
 *
 * @param {string} username The username to generate UUID for.
...
function generateOfflineUUID(username) {
    ...
    md5Bytes[8] = (md5Bytes[8] & 0x3f) | 0x80 // Variant
    const hex = md5Bytes.toString('hex')
    return hex.substring(0, 8) + '-' + hex.substring(8, 12) + '-' + hex.substring(12, 16) + '-' + hex.substring(16, 20) + '-' + hex.substring(20)
}

/**
 * Add an offline account. This creates a local account without authentication.
 * The account will have a generated UUID based on the username.
 *
 * @param {string} username The desired username (3-16 characters).
...
exports.addOfflineAccount = async function(username) {
    try {
        ...
        return Promise.reject({
            title: Lang.queryJS('auth.offline.error.unknownTitle'),
            desc: Lang.queryJS('auth.offline.error.unknownDesc')
        })
    }
}
